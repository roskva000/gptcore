import {
  ...
  DRIFT_CLEAR_CLIMB_ASCENT_WINDOW_SECONDS,
  ...
  KILLBOX_LOCK_DRAG_ROTATION_DEGREES,
...
export const createSurvivalSnapshotReport = (): SurvivalSnapshotReport => {
  ...
  return {
    ...
    controller: `center-seeking avoidance heuristic with 180ms reaction interval, +${OPENING_REQUIRED_SPAWN_DISTANCE_BONUS}px opening spawn distance through ${OPENING_REQUIRED_SPAWN_DISTANCE_CUTOFF_SECONDS}s, projected-path forward-alignment rerolls above ${EARLY_FORWARD_SPAWN_ALIGNMENT_THRESHOLD.toFixed(1)} dot through ${EARLY_FORWARD_SPAWN_REROLL_CUTOFF_SECONDS}s (${EARLY_FORWARD_SPAWN_ALIGNMENT_PENALTY}px-equivalent penalty), projected-path lane-stack rerolls within ${EARLY_LANE_STACK_DISTANCE}px above ${EARLY_LANE_STACK_ALIGNMENT_THRESHOLD.toFixed(2)} dot through ${EARLY_LANE_STACK_REROLL_CUTOFF_SECONDS}s (${EARLY_LANE_STACK_PENALTY}px-equivalent penalty), projected-path threat-crowding rerolls within ${EARLY_THREAT_CROWDING_DISTANCE}px above ${EARLY_THREAT_CROWDING_ALIGNMENT_THRESHOLD.toFixed(1)} dot through ${EARLY_THREAT_CROWDING_REROLL_CUTOFF_SECONDS}s (${EARLY_THREAT_CROWDING_PENALTY}px-equivalent penalty), same-edge spawn-column rerolls within ${EARLY_SPAWN_EDGE_CLUSTER_LATERAL_DISTANCE}px lateral / ${EARLY_SPAWN_EDGE_CLUSTER_DEPTH}px depth through ${EARLY_SPAWN_EDGE_CLUSTER_REROLL_CUTOFF_SECONDS}s (${EARLY_SPAWN_EDGE_CLUSTER_PENALTY}px-equivalent penalty), near-player same-edge rerolls within ${EARLY_PRESSURED_SAME_EDGE_PLAYER_DISTANCE}px and ${EARLY_PRESSURED_SAME_EDGE_LATERAL_DISTANCE}px lateral below score ${EARLY_PRESSURED_SPAWN_ACCEPT_SCORE} through ${EARLY_PRESSURED_SPAWN_REROLL_CUTOFF_SECONDS}s, deep same-side follow-up sweeps stay reroll-eligible out to ${EARLY_PRESSURED_SAME_SIDE_LATERAL_DISTANCE}px, retreat-pinch rerolls within ${EARLY_RETREAT_PINCH_PLAYER_DISTANCE}px above ${EARLY_RETREAT_PINCH_ALIGNMENT_THRESHOLD.toFixed(2)} forward alignment when the new spawn seals the rear lane within ${EARLY_RETREAT_PINCH_RETREAT_LATERAL_DISTANCE}px through ${EARLY_RETREAT_PINCH_REROLL_CUTOFF_SECONDS}s, mid-run projected-stack rerolls within ${MID_RUN_PROJECTED_STACK_PLAYER_DISTANCE}px above ${MID_RUN_PROJECTED_STACK_ALIGNMENT_THRESHOLD.toFixed(2)} alignment from ${MID_RUN_PROJECTED_STACK_REROLL_START_SECONDS}s to ${MID_RUN_PROJECTED_STACK_REROLL_CUTOFF_SECONDS}s, breakthrough forces a ${BREAKTHROUGH_STRAFE_FORK_WINDOW_SECONDS.toFixed(1)}s strafe fork from ${STRAFE_OBSTACLE_UNLOCK_SECONDS}s at ${BREAKTHROUGH_STRAFE_FORK_ROTATION_DEGREES}deg cross-lane travel, then a ${BREAKTHROUGH_HINGE_FEINT_WINDOW_SECONDS.toFixed(1)}s hinge feint from ${BREAKTHROUGH_HINGE_FEINT_WINDOW_START_SECONDS.toFixed(1)}s at ${BREAKTHROUGH_HINGE_FEINT_ROTATION_DEGREES}deg with ${BREAKTHROUGH_HINGE_FEINT_TARGET_LAG_SECONDS.toFixed(2)}s target lag, then a ${BREAKTHROUGH_SURGE_SNAP_WINDOW_SECONDS.toFixed(1)}s surge snap from ${SURGE_OBSTACLE_UNLOCK_SECONDS}s at ${BREAKTHROUGH_SURGE_SNAP_ROTATION_DEGREES}deg with ${BREAKTHROUGH_SURGE_SNAP_TARGET_LEAD_SECONDS.toFixed(2)}s forward lead, then a ${BREAKTHROUGH_GATE_CUT_WINDOW_SECONDS.toFixed(1)}s gate cut from ${BREAKTHROUGH_GATE_CUT_WINDOW_START_SECONDS.toFixed(1)}s at ${BREAKTHROUGH_GATE_CUT_ROTATION_DEGREES}deg with ${BREAKTHROUGH_GATE_CUT_TARGET_LEAD_SECONDS.toFixed(2)}s forward lead before cadence resumes, strafe obstacles every ${STRAFE_OBSTACLE_CADENCE}th spawn from ${STRAFE_OBSTACLE_UNLOCK_SECONDS}s with ${STRAFE_OBSTACLE_ROTATION_DEGREES}deg cross-lane travel, surge obstacles every ${SURGE_OBSTACLE_CADENCE}th spawn from ${SURGE_OBSTACLE_UNLOCK_SECONDS}s with ${SURGE_OBSTACLE_SPEED_MULTIPLIER.toFixed(2)}x speed, killbox onset forces a ${KILLBOX_FORCED_LEAD_WINDOW_SECONDS.toFixed(1)}s lead cut with ${KILLBOX_FORCED_LEAD_TARGET_LEAD_SECONDS.toFixed(2)}s forward target lead, then a ${KILLBOX_ECHO_FOLLOW_THROUGH_WINDOW_SECONDS.toFixed(1)}s echo follow-through with ${KILLBOX_ECHO_FOLLOW_THROUGH_ROTATION_DEGREES}deg scissor travel, a ${KILLBOX_PINCH_LOCK_WINDOW_SECONDS.toFixed(1)}s pinch lock from ${KILLBOX_PINCH_LOCK_WINDOW_START_SECONDS.toFixed(1)}s at ${KILLBOX_PINCH_LOCK_ROTATION_DEGREES}deg with ${KILLBOX_PINCH_LOCK_TARGET_LEAD_SECONDS.toFixed(2)}s forward target lead, a ${KILLBOX_ECHO_BRIDGE_WINDOW_SECONDS.toFixed(1)}s bridge echo at ${KILLBOX_ECHO_BRIDGE_WINDOW_START_SECONDS.toFixed(1)}s with ${KILLBOX_ECHO_BRIDGE_ROTATION_DEGREES}deg travel, a ${KILLBOX_SEAL_SNAP_WINDOW_SECONDS.toFixed(1)}s seal snap from ${KILLBOX_SEAL_SNAP_WINDOW_START_SECONDS.toFixed(1)}s at ${KILLBOX_SEAL_SNAP_ROTATION_DEGREES}deg with ${KILLBOX_SEAL_SNAP_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${KILLBOX_ECHO_HANDOFF_WINDOW_SECONDS.toFixed(1)}s echo lock-in from ${ECHO_OBSTACLE_UNLOCK_SECONDS}s with ${KILLBOX_ECHO_HANDOFF_ROTATION_DEGREES}deg travel, then a ${KILLBOX_FOLD_SNAP_WINDOW_SECONDS.toFixed(1)}s fold snap from ${KILLBOX_FOLD_SNAP_WINDOW_START_SECONDS.toFixed(1)}s at ${KILLBOX_FOLD_SNAP_ROTATION_DEGREES}deg with ${KILLBOX_FOLD_SNAP_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${KILLBOX_LOCK_DRAG_WINDOW_SECONDS.toFixed(1)}s lock drag from ${KILLBOX_LOCK_DRAG_WINDOW_START_SECONDS.toFixed(1)}s at ${KILLBOX_LOCK_DRAG_ROTATION_DEGREES}deg with ${KILLBOX_LOCK_DRAG_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${KILLBOX_SLACK_CUT_WINDOW_SECONDS.toFixed(1)}s slack cut from ${KILLBOX_SLACK_CUT_WINDOW_START_SECONDS.toFixed(1)}s at ${KILLBOX_SLACK_CUT_ROTATION_DEGREES}deg with ${KILLBOX_SLACK_CUT_TARGET_LAG_SECONDS.toFixed(2)}s lag before drift release cracks the lane at ${DRIFT_OBSTACLE_UNLOCK_SECONDS.toFixed(1)}s, lead obstacles every ${LEAD_OBSTACLE_CADENCE}th spawn from ${LEAD_OBSTACLE_UNLOCK_SECONDS}s with ${LEAD_OBSTACLE_TARGET_LEAD_SECONDS.toFixed(2)}s forward target lead, echo obstacles every ${ECHO_OBSTACLE_CADENCE}th spawn from ${ECHO_OBSTACLE_UNLOCK_SECONDS}s with ${ECHO_OBSTACLE_TARGET_LAG_SECONDS.toFixed(2)}s target lag, drift obstacles every ${DRIFT_OBSTACLE_CADENCE}th spawn from ${DRIFT_OBSTACLE_UNLOCK_SECONDS}s with a ${DRIFT_RELEASE_FOLD_CARRY_WINDOW_SECONDS.toFixed(1)}s fold-carry cut at ${DRIFT_RELEASE_FOLD_CARRY_ROTATION_DEGREES}deg and ${KILLBOX_FOLD_SNAP_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${(DRIFT_RELEASE_WINDOW_SECONDS - DRIFT_RELEASE_FOLD_CARRY_WINDOW_SECONDS).toFixed(1)}s release stretch at ${DRIFT_RELEASE_ROTATION_DEGREES}deg with ${DRIFT_RELEASE_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${DRIFT_REBOUND_HOLD_WINDOW_SECONDS.toFixed(1)}s rebound hold at ${DRIFT_REBOUND_ROTATION_DEGREES}deg with ${DRIFT_REBOUND_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${DRIFT_REBOUND_CROSS_WINDOW_SECONDS.toFixed(2)}s rebound cross at ${DRIFT_REBOUND_CROSS_ROTATION_DEGREES}deg with ${DRIFT_REBOUND_CROSS_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${(DRIFT_REBOUND_WINDOW_SECONDS - DRIFT_REBOUND_HOLD_WINDOW_SECONDS - DRIFT_REBOUND_CROSS_WINDOW_SECONDS).toFixed(2)}s rebound punish at ${DRIFT_REBOUND_PUNISH_ROTATION_DEGREES}deg with ${DRIFT_REBOUND_PUNISH_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${(DRIFT_SWEEP_WINDOW_SECONDS - DRIFT_SWEEP_LOCK_WINDOW_SECONDS).toFixed(1)}s late sweep from ${DRIFT_SWEEP_WINDOW_START_SECONDS.toFixed(1)}s at ${DRIFT_SWEEP_ROTATION_DEGREES}deg with ${DRIFT_SWEEP_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${DRIFT_SWEEP_LOCK_WINDOW_SECONDS.toFixed(1)}s sweep lock at ${(DRIFT_SWEEP_WINDOW_START_SECONDS + DRIFT_SWEEP_WINDOW_SECONDS - DRIFT_SWEEP_LOCK_WINDOW_SECONDS).toFixed(1)}s with ${DRIFT_SWEEP_LOCK_ROTATION_DEGREES}deg travel and ${DRIFT_SWEEP_LOCK_TARGET_LAG_SECONDS.toFixed(2)}s lag before a ${DRIFT_AFTERSHOCK_WINDOW_SECONDS.toFixed(1)}s aftershock clamp at ${DRIFT_AFTERSHOCK_ROTATION_DEGREES}deg with ${DRIFT_AFTERSHOCK_TARGET_LAG_SECONDS.toFixed(2)}s lag, followed by a ${DRIFT_RECENTER_WINDOW_SECONDS.toFixed(1)}s recenter handoff at ${DRIFT_RECENTER_ROTATION_DEGREES}deg with ${DRIFT_RECENTER_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${DRIFT_CENTER_PIN_WINDOW_SECONDS.toFixed(1)}s center pin at ${(DRIFT_CLEAR_CLIMB_WINDOW_START_SECONDS - DRIFT_PRECLEAR_WINDOW_SECONDS - DRIFT_FALSE_CLEAR_WINDOW_SECONDS - DRIFT_CENTER_PIN_WINDOW_SECONDS).toFixed(1)}s with ${DRIFT_CENTER_PIN_ROTATION_DEGREES}deg travel and ${DRIFT_CENTER_PIN_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${DRIFT_FALSE_CLEAR_WINDOW_SECONDS.toFixed(1)}s false-clear bait at ${(DRIFT_CLEAR_CLIMB_WINDOW_START_SECONDS - DRIFT_PRECLEAR_WINDOW_SECONDS - DRIFT_FALSE_CLEAR_WINDOW_SECONDS).toFixed(1)}s with ${DRIFT_FALSE_CLEAR_ROTATION_DEGREES}deg travel and ${DRIFT_FALSE_CLEAR_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${DRIFT_PRECLEAR_WINDOW_SECONDS.toFixed(1)}s preclear squeeze at ${(DRIFT_CLEAR_CLIMB_WINDOW_START_SECONDS - DRIFT_PRECLEAR_WINDOW_SECONDS).toFixed(1)}s with ${DRIFT_PRECLEAR_ROTATION_DEGREES}deg travel and ${DRIFT_PRECLEAR_TARGET_LAG_SECONDS.toFixed(2)}s lag, then forced clear-climb drift from ${DRIFT_CLEAR_CLIMB_WINDOW_START_SECONDS.toFixed(1)}s with a ${DRIFT_CLEAR_CLIMB_ASCENT_WINDOW_SECONDS.toFixed(1)}s ascent stair at ${DRIFT_CLEAR_CLIMB_ASCENT_ROTATION_DEGREES}deg and ${DRIFT_CLEAR_CLIMB_ASCENT_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${DRIFT_CLEAR_CLIMB_LEDGE_WINDOW_SECONDS.toFixed(1)}s ledge feint at ${(DRIFT_CLEAR_CLIMB_WINDOW_START_SECONDS + DRIFT_CLEAR_CLIMB_ASCENT_WINDOW_SECONDS).toFixed(1)}s with ${DRIFT_CLEAR_CLIMB_LEDGE_ROTATION_DEGREES}deg and ${DRIFT_CLEAR_CLIMB_LEDGE_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${DRIFT_CLEAR_CLIMB_RIDGE_WINDOW_SECONDS.toFixed(1)}s ridge cut at ${(DRIFT_CLEAR_CLIMB_WINDOW_START_SECONDS + DRIFT_CLEAR_CLIMB_ASCENT_WINDOW_SECONDS + DRIFT_CLEAR_CLIMB_LEDGE_WINDOW_SECONDS).toFixed(1)}s with ${DRIFT_CLEAR_CLIMB_RIDGE_ROTATION_DEGREES}deg and ${DRIFT_CLEAR_CLIMB_RIDGE_TARGET_LAG_SECONDS.toFixed(2)}s lag, a ${DRIFT_CLEAR_CLIMB_CREST_WINDOW_SECONDS.toFixed(1)}s crest veer at ${(DRIFT_CLEAR_CLIMB_WINDOW_START_SECONDS + DRIFT_CLEAR_CLIMB_ASCENT_WINDOW_SECONDS + DRIFT_CLEAR_CLIMB_LEDGE_WINDOW_SECONDS + DRIFT_CLEAR_CLIMB_RIDGE_WINDOW_SECONDS).toFixed(1)}s with ${DRIFT_CLEAR_CLIMB_CREST_ROTATION_DEGREES}deg and ${DRIFT_CLEAR_CLIMB_CREST_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a summit snap at ${DRIFT_CLEAR_CLIMB_SUMMIT_ROTATION_DEGREES}deg with ${DRIFT_CLEAR_CLIMB_SUMMIT_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${OVERTIME_OPENER_BANKED_WINDOW_SECONDS.toFixed(1)}s banked-air reopen from ${SURVIVAL_GOAL_SECONDS.toFixed(1)}s at ${OVERTIME_OPENER_BANKED_ROTATION_DEGREES}deg with ${OVERTIME_OPENER_BANKED_TARGET_LAG_SECONDS.toFixed(2)}s lag before a ${OVERTIME_OPENER_CASHOUT_WINDOW_SECONDS.toFixed(1)}s cash-out snap at ${(SURVIVAL_GOAL_SECONDS + OVERTIME_OPENER_BANKED_WINDOW_SECONDS).toFixed(1)}s with ${OVERTIME_OPENER_CASHOUT_ROTATION_DEGREES}deg and ${OVERTIME_OPENER_CASHOUT_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${OVERTIME_CARRY_WINDOW_SECONDS.toFixed(1)}s house cut from ${(SURVIVAL_GOAL_SECONDS + OVERTIME_OPENER_BANKED_WINDOW_SECONDS + OVERTIME_OPENER_CASHOUT_WINDOW_SECONDS).toFixed(1)}s at ${OVERTIME_CARRY_ROTATION_DEGREES}deg with ${OVERTIME_CARRY_TARGET_LAG_SECONDS.toFixed(2)}s lag, then a ${OVERTIME_DUE_NOW_WINDOW_SECONDS.toFixed(1)}s due-now snap from ${(SURVIVAL_GOAL_SECONDS + OVERTIME_OPENER_BANKED_WINDOW_SECONDS + OVERTIME_OPENER_CASHOUT_WINDOW_SECONDS + OVERTIME_CARRY_WINDOW_SECONDS).toFixed(1)}s at ${OVERTIME_DUE_NOW_ROTATION_DEGREES}deg with ${OVERTIME_DUE_NOW_TARGET_LAG_SECONDS.toFixed(2)}s lag, ${EARLY_SPAWN_TARGET_LAG_SECONDS.toFixed(2)}s early spawn target lag through ${EARLY_SPAWN_TARGET_LAG_CUTOFF_SECONDS}s, ${EARLY_SPAWN_COLLISION_GRACE_MS}ms collision grace through ${EARLY_SPAWN_COLLISION_GRACE_CUTOFF_SECONDS}s, ${OBSTACLE_COLLISION_RADIUS}px visible-arena hit margin, and ${OFFSCREEN_CULL_MARGIN}px offscreen cull margin`,
