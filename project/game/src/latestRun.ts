export const latestRunSummary = {
  ...
  title: 'Deaths now point both ways',
  intro:
    'The last meaningful run stayed inside death readability: the fatal obstacle keeps its in-scene spotlight tag plus connector, and both the red impact ray and teal BREAK guide now end with arrowheads so the hit lane and escape lane read faster.',
  bullets: [
    'Game over still shows the fatal lane plus the teal BREAK guide toward the safer opposite lane.',
    'The red impact ray now ends with an arrowhead so the incoming fatal lane reads faster at a glance.',
    'The teal BREAK guide now also ends with an arrowhead so the first escape direction is more obvious on retry.',
