...
hit_feedback_status:
current: visual death feedback, fatal lane impact ray, directional hit callout, dedicated fatal-lane callout, killer obstacle spotlight with in-scene `KILLER` tag, non-fatal threat dimming, teal escape ray + marker, `BREAK ...` escape prompt, and short procedural death blip active after user interaction
baseline: expanded in Run #36 without changing deterministic telemetry baselines
...
baseline: copy refreshed in Run #36 to match the in-scene killer tag pass
...
target: 5-10 runs via session telemetry when a suitable browser runtime is available; note whether replay really restarts on one action and whether killer tag + threat dimming + teal guide + `BREAK ...` prompt + fatal-lane callout + ray + directional hit feedback improve fairness/readability

telemetry_regression_check:
current: passes via `npm run telemetry:check` as of Run #36
...
- current tuning signal: deterministic survival baseline korunurken olum aninin okunurlugu yeni killer tag + killer spotlight + threat dimming + teal escape guide paketi ile guclendi; insan oyuncu algisi icin manuel sample hala gerekli
