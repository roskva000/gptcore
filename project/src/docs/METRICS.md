...
baseline: 26.5s
target: hold `>= 26.5s` while checking manually that the faster `20s+` chase plus the forward-pressure and lane-stack spawn filters still feel fair and readable
...
current: 145 at 0s, 183 at 10s, 217 at 20s, 254 at 30s, 310 at 45s, 320 at 60s
baseline: Run #70 raised only the `20s+` speed slope to reduce midgame obstacle buildup without changing pacing or opening-fairness guards
target: preserve the early-death guard and validate manually that the faster `20s+` chase clears space instead of feeling unfairly sharp
...
baseline: Run #54 aligned the export with actual early-death risk; Run #55 aligned sample reset with that export by clearing stale saved summaries instead of carrying them into a fresh session; Run #61 aligned `first death` with the minimum death in the sample instead of the first chronological run; Run #74 moved the checked deterministic baseline text to `26.5s avg / 6.3s first death / 4% early` while the 5-seed validation sample average stayed `24.1s`
...
current: `npm run telemetry:check` passes on the Run #74 baseline and still asserts fresh-session retry remains `null`, same-session retry delay is still tracked, and the deterministic survival proxy still models runtime visible-arena hit guard plus `96px` offscreen cull margin
baseline: as of Run #74 asserts pacing, required spawn distance, forward-pressure and lane-stack spawn reroll behavior through the checked snapshot, faster `20s+` speed anchors, survival, survival buckets, honest validation summary/report wording, early spawn collision grace, the narrower obstacle collider, retry-delay session integrity, and runtime-aligned collision/cull proxy behavior
...
- current tuning signal: deterministic snapshot artik `26.5s / 6.3s / 4%`; Run #74 lane-stack filter avg'i artirdi ama opener'da bir `<10s` outlier hala var, Run #67'nin daralttigi obstacle collider ise bu metrigi degistirmeden insan hissinde fairness kazanimi ariyor
