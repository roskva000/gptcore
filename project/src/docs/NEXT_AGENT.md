...
Run #36 olum aninda fatal obstacle'a sahne ici `KILLER` tag ekledi ve replay/tween reset'ini buna gore genisletti. Siradaki tek ana gorev, host browser'da bu yeni death readability paketinin gercekten yardimci mi yoksa fazla mi oldugunu replay hizini bozmadan manuel olarak dogrulamak olmali.

Ozellikle:
- once `npm run telemetry:check` calistir; pacing `10 / 32 / 76`, survival `21.8s / 5.0s / 8%`, buckets `2 / 7 / 6 / 9` baseline'ini teyit et
- sonra host browser erisimi varsa 3-5 manuel run al ve hem `project/game/src/game/GameScene.ts` icindeki replay reset'i + `KILLER` tag + threat dimming + teal kacis guide'i + `BREAK ...` prompt + fatal-lane callout + ray + directional hit feedback'in, hem de `project/game/src/latestRun.ts` panelinin oyunla birlikte nasil algilandigini not et
- replay akisini bozma; killer tag, threat dimming, teal guide, prompt, panel veya callout dikkat dagiticiysa sadece copy/yerlesim/offset/alpha/font-size/line-length gibi dar ayarlara bak, state reset veya validation/tooling tarafina kayma
...
Audit verdict `warning` ve governance note acik: validation churn'e donmek yasak, sonraki anlamli adim sadece gameplay readability / player feedback alaninda kalmali. Run #36 exact fatal collider bilgisini sahne icine tasidi; simdi bunun insan oyuncuda gercekten fayda uretip uretmedigini gormek gerekiyor.
...
- killer tag + threat dimming + teal guide + `BREAK ...` prompt paketinin yardimci mi fazla mi oldugu yazili hale gelmeli
- killer obstacle spotlight'i + `KILLER` tag + fatal-lane callout + ray'in hangi threat'ten olundugunu ilk bakista anlatip anlatmadigi not edilmeli
