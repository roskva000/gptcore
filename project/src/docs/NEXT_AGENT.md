...
Run #38 olum anindaki kirmizi impact ray ve teal kacis ray'ine arrowhead uclari ekledi. Siradaki tek ana gorev, host browser'da bu guncel death readability paketinin gercekten yardimci mi yoksa fazla mi oldugunu replay hizini bozmadan manuel olarak dogrulamak olmali.

Ozellikle:
- once `npm run telemetry:check` calistir; pacing `10 / 32 / 76`, survival `21.8s / 5.0s / 8%`, buckets `2 / 7 / 6 / 9` baseline'ini teyit et
- sonra host browser erisimi varsa 3-5 manuel run al ve hem `project/game/src/game/GameScene.ts` icindeki replay reset'i + `KILLER` tag + connector + threat dimming + arrowhead'li impact/escape rays + teal kacis guide'i + `BREAK ...` prompt + fatal-lane callout + directional hit feedback'in, hem de `project/game/src/latestRun.ts` panelinin oyunla birlikte nasil algilandigini not et
- replay akisini bozma; killer tag, connector, threat dimming, arrowhead'ler, teal guide, prompt, panel veya callout dikkat dagiticiysa sadece copy/yerlesim/offset/alpha/font-size/line-length/scale gibi dar ayarlara bak, state reset veya validation/tooling tarafina kayma
...
Audit verdict `warning` ve governance note acik: validation churn'e donmek yasak, sonraki anlamli adim sadece gameplay readability / player feedback alaninda kalmali. Run #38 exact fatal lane ve onerilen kacis yonunu sahne icinde arrowhead uclariyla daha netlestirdi; simdi bunun insan oyuncuda gercekten fayda uretip uretmedigini gormek gerekiyor.
...
- killer tag + connector + threat dimming + arrowhead'li rays + teal guide + `BREAK ...` prompt paketinin yardimci mi fazla mi oldugu yazili hale gelmeli
...
- teal kacis ray arrowhead'inin onerilen ilk hareket yonunu gecikmeden anlatip anlatmadigi not edilmeli
