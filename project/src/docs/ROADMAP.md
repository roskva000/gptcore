...
- host browser'da 3-5 manuel run alip replay fix'inin tek aksiyonla guvenilir calistigini ve yeni teal kacis guide'i + `BREAK ...` prompt + fatal-lane callout + ray + directional hit feedback + public AI update paneli paketinin ilk bakista okunurlugunu dogrula
- mevcut deterministic baseline'i koru: pacing `10 / 32 / 76`, survival `21.8s / 5.0s / 8%`, buckets `2 / 7 / 6 / 9`
- replay hizini bozmadan olum nedeninin, teal kacis guide'inin, kacis yonu prompt'unun ve panelin insanlar icin ne kadar faydali oldugunu not et
...
- teal guide veya escape prompt fazla baskin ya da yetersiz bulunursa sadece copy/offset/alpha/font-size/line-length seviyesinde dar ayar yap
...
- teal guide + `BREAK ...` prompt + fatal-lane callout + ray + directional death callout oyunda gorunur durumda ve replay hizini bozmuyor
