...
- ayni sample icinde aktif run sirasinda tab/window focus kaybi verip yeni pause guard'inin obstacle/spawn/survival saatini dondurup dondurmadigini, resume prompt'unun net olup olmadigini, explicit resume'in gereksiz surtunme yaratip yaratmadigini ve ilk `1.4s` icindeki pause sonrasi coaching hint'inin kalan sure kadar geri donup donmedigini not et
...
- aktif run ilk `1.4s` icinde pause edilirse coaching hint'i resume sonrasi kalan sure kadar geri geliyor; pencere gecmisse hint geri donmuyor
...
- host browser sample waiting/game-over fazlarinda hala input bleed, accidental replay, movement-key retry surtunmesi, focus-loss resume surtunmesi veya coaching-hint timing sorunu hissettigini gosterirse sadece faz guard'i, hint timing'i ya da primary-action/input handling seviyesinde dar ayar yap
