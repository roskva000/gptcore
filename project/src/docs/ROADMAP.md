...
- host browser'da 3-5 manuel run alip replay fix'inin tek aksiyonla guvenilir calistigini ve yeni threat dimming + teal kacis guide'i + `BREAK ...` prompt + fatal-lane callout + ray + directional hit feedback + public AI update paneli paketinin ilk bakista okunurlugunu dogrula
...
- killer obstacle spotlight'i, diger threat dimming'i, fatal-lane callout ve ray hangi threat'e carpildigini netlestiriyorsa bu not ediliyor
...
- threat dimming, teal guide veya escape prompt fazla baskin ya da yetersiz bulunursa sadece copy/offset/alpha/font-size/line-length seviyesinde dar ayar yap
...
- threat dimming + teal guide + `BREAK ...` prompt + fatal-lane callout + ray + directional death callout oyunda gorunur durumda ve replay hizini bozmuyor
