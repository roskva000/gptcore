...
- host browser'da 3-5 manuel run alip replay fix'inin tek aksiyonla guvenilir calistigini ve yeni `KILLER` spotlight etiketi + connector + threat dimming + arrowhead'li impact ray + arrowhead'li teal kacis guide'i + `BREAK ...` prompt + fatal-lane callout + directional hit feedback + public AI update paneli paketinin ilk bakista okunurlugunu dogrula
- mevcut deterministic baseline'i koru: pacing `10 / 32 / 76`, survival `21.8s / 5.0s / 8%`, buckets `2 / 7 / 6 / 9`
- replay hizini bozmadan olum nedeninin, killer tag connector'unun, threat dimming'in, yeni arrowhead uclarinin, teal kacis guide'inin, kacis yonu prompt'unun ve panelin insanlar icin ne kadar faydali oldugunu not et
...
- impact ve escape ray arrowhead'leri, gelen tehlike ile onerilen kacis yonunu bir bakista okuyorsa bu not ediliyor
...
- killer tag connector'u, threat dimming, arrowhead'li rays, teal guide veya escape prompt fazla baskin ya da yetersiz bulunursa sadece copy/offset/alpha/font-size/line-length/scale seviyesinde dar ayar yap
...
- killer tag + connector + threat dimming + arrowhead'li impact/escape rays + teal guide + `BREAK ...` prompt + fatal-lane callout + directional death callout oyunda gorunur durumda ve replay hizini bozmuyor
