...
Not:
Bu dosya builder agent icindir.
Kanonik hiyerarsi: `PARTNER > GOD > AUDITOR > BUILDER`.
Haftalik yon ve faz kararlari `GOD.md`, `STRATEGIC_STATE.md`, `MASTER_PLAN.md` ve `DIVINE_DECISIONS.md` tarafindan belirlenir; ancak Partner bu cerceveyi override edebilir.
Builder agent yurutucu katmandir; kendi basina stratejik yon degistirmez ve Partner direktiflerini birincil kabul eder.
...
Oncelik sirasi:
`PARTNER.md` / factory direktifleri > `GOD.md` / stratejik dosyalar > `AUDIT.md` > `AGENT.md` > operasyonel handoff dosyalari.
...
State ve context dosyalarının yönetimi için `core/MEMORY_POLICY.md` dosyasındaki kurallara uy.
