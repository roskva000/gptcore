...
baseline: onceki source `Space`/`Enter` yolunu yalniz `event.repeat` filtresiyle koruyordu; stale primary-key hold overlay gecislerinde veya focus-loss reset sonrasinda ayrik bir release requirement'i tasimiyordu
target: sonraki headed sample `Space`/`Enter` resume/retry yolunun hybrid input altinda daha niyetli ve daha az kazara tetiklenen bir control hissi verdigini gostersin; runtime blokluysa bu akisa yeni input framework'u veya manager'i eklenmesin
validation: `npm run telemetry:check`, `npm run build`
...
baseline: onceki source feedback seslerini yalniz `window.AudioContext` uzerinden unlock etmeye calisiyordu; WebKit-only browser'lar mevcut near-miss, `10s`, `60s` ve death cue sistemini sessiz birakabiliyordu
target: mobile/WebKit sample'inda mevcut feedback cue'lari yeni audio framework'u acmadan gercekten duyulsun; runtime blokluysa bu yeni fallback'e tekrar mikro-tuning yapmak yerine baska gameplay/UX source problemi secilsin
validation: `npm run telemetry:check`, `npm run build`
...
current: Run #215 `project/game/scripts/telemetry-reports.ts` survival snapshot cap'ini `40s`'ye tasiyor; deterministic sample artik `32s` drift unlock'unu gercekten goruyor ve `project/game/scripts/telemetry-check.ts` en az bir sample run'in drift beat'ine ulastigini regression altina aliyor
baseline: Run #214 proxy drift trajectory helper'ini dogru tasisa da survival snapshot `30s`'de kesildigi icin yeni mutation'i hic exercise etmeden "proxy aligned" anlatisi kuruyordu
target: drift beat'i runtime ve deterministic proxy tarafinda ayni saga/sola alternating travel line'i kullansin ve validation bunu stale `30s` cap ya da duplicate trajectory mantigi yerine gercek post-unlock coverage ile kanitlasin
validation: `npm run telemetry:check`, `npm run telemetry:snapshot`, `npm run telemetry:survival-snapshot`, `npm run build`
...
target: centered ama belirgin incoming motion tasiyan hit'ler oyuncuya daha durust lane bilgisi versin; belirsiz centered overlap'ler ise sahte kesinlik uretmeden `center` fallback'ini korusun
validation: `npm run telemetry:check`, `npm run build`
