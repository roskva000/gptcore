...
current: `V` export now produces `5 runs | first death 6.3s | early 20% | 5/5 runs, review early deaths`, counts `runs=` from completed sample deaths instead of raw start count, and the waiting/playing HUD progress satirlari da ayni completed-run semantigini kullaniyor; `R` reset still clears the saved export so HUD summary returns to `not saved yet`
...
target: keep `V` export, parser, and in-game progress aligned, count validation sample size from completed runs only, never mark a 5-run sample as healthy while it still contains `<10s` deaths, never let a fresh reset show a stale export as if it belongs to the new sample, and keep the embedded deterministic baseline text synchronized with the latest checked gameplay snapshot
