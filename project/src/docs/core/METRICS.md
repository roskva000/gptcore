...
current: `npm run telemetry:check` passes on the Run #83 baseline and still asserts fresh-session retry remains `null`, same-session retry delay is still tracked, the deterministic survival proxy still models runtime visible-arena hit guard plus `96px` offscreen cull margin, offscreen obstacle'larin lane-stack reroll tetiklememesi guard altinda, projected-path forward-pressure preserved left-edge opener senaryosu sabit, and seed `#3`un ilk alti spawn trace'i sabit kaliyor
baseline: as of Run #83 asserts pacing, required spawn distance, projected-path forward-pressure and lane-stack spawn reroll behavior through the checked snapshot, visible-only lane-stack guard'ini, softened `20s+` speed anchors, survival, survival buckets, honest validation summary/report wording, early spawn collision grace, the narrower obstacle collider, retry-delay session integrity, runtime-aligned collision/cull proxy behavior, and the seed `#3` outlier trace
...
- current tuning signal: deterministic snapshot artik `26.6s / 6.3s / 4%`; Run #78 projected-path lane-stack referansi average spawn reroll'u `0.4`e indirdi, Run #83 ayni projected-path mantigini forward-pressure scoring'e de tasidi, ama opener'da bir `<10s` outlier hala var
