...
current: `npm run telemetry:check` passes on the Run #89 baseline and still asserts fresh-session retry remains `null`, same-session retry delay is still tracked, the deterministic survival proxy still models runtime `11px visible-arena hit margin` plus `96px` offscreen cull margin, offscreen obstacle'larin lane-stack reroll tetiklememesi guard altinda, partial-visible edge obstacle'larin da lane-stack reroll tetiklememesi guard altinda, projected-path forward-pressure preserved left-edge opener senaryosu sabit, wall-edge projected-path reference arena icine clamp'leniyor, death-direction helper ayni-yon chase carpismasinda gercek threat lane'i koruyor, merkez-overlap durumda velocity fallback'i calisiyor, and seed `#3`un ilk alti spawn trace'i yeni visible-count farki ile sabit kaliyor
baseline: as of Run #89 asserts pacing, required spawn distance, projected-path forward-pressure and lane-stack spawn reroll behavior through the checked snapshot, the wall-edge projected-path clamp, visible-only ve full-collider-visible lane-stack guard'larini, slightly firmer `20s+` speed anchors, survival, survival buckets, honest validation summary/report wording, early spawn collision grace, the narrower obstacle collider, full visible-arena hit margin, retry-delay session integrity, runtime-aligned collision/cull proxy behavior, death-direction chase/collision readability guards, and the seed `#3` outlier trace
...
- current tuning signal: deterministic snapshot artik `26.5s / 6.3s / 4%`; Run #78 projected-path lane-stack referansi average spawn reroll'u `0.4`e indirdi, Run #83 ayni projected-path mantigini forward-pressure scoring'e de tasidi, Run #84 bu projected-path referansini duvar-kenarinda arena icine clamp'ledi, Run #87 ise `20s+` chase'i kucuk bir kademe tekrar sikilastirdi; Run #88 de tam gorunmeden gelen edge hit'leri kapatan `11px visible-arena hit margin` ekledi; ama opener'da bir `<10s` outlier hala var
