...
current: Run #96 makes waiting/game-over `tap/click` primary actions start or retry the run without also turning that same press into active pointer steering; steering now waits for pointer release, while intentional held-pointer start/retry still steers immediately after held confirmation
baseline: before Run #96 a discrete pointer start/retry could hand the same press straight into runtime steering on the first playing frame, which risked dragging the player toward the HUD/click point even when the player's intent was only to begin the next run
target: confirm manually that tap/click start and retry now feel neutral and controlled, without adding friction for deliberate hold-to-steer players on desktop or touch devices
