...
current: `npm run telemetry:check` passes on the Run #91 baseline and still asserts fresh-session retry remains `null`, same-session retry delay is still tracked, the deterministic survival proxy still models runtime `11px visible-arena hit margin` plus `96px` offscreen cull margin, offscreen obstacle'larin lane-stack reroll tetiklememesi guard altinda, partial-visible edge obstacle'larin da lane-stack reroll tetiklememesi guard altinda, projected-path forward-pressure preserved left-edge opener senaryosu sabit, wall-edge projected-path reference arena icine clamp'leniyor, death-direction helper ayni-yon chase carpismasinda gercek threat lane'i koruyor, merkez-overlap durumda velocity fallback'i calisiyor, top-edge impact ve bottom-edge fatal callout placement'lari arena icinde kalacak sekilde guard altinda, and seed `#3`un ilk alti spawn trace'i yeni visible-count farki ile sabit kaliyor
baseline: as of Run #91 asserts pacing, required spawn distance, projected-path forward-pressure and lane-stack spawn reroll behavior through the checked snapshot, the wall-edge projected-path clamp, visible-only ve full-collider-visible lane-stack guard'larini, slightly firmer `20s+` speed anchors, survival, survival buckets, honest validation summary/report wording, early spawn collision grace, the narrower obstacle collider, full visible-arena hit margin, retry-delay session integrity, runtime-aligned collision/cull proxy behavior, death-direction chase/collision readability guards, edge-safe death callout placement, and the seed `#3` outlier trace
