...
Interactive browser/runtime varsa `5-10` manuel run topla ve ilk structured girdiyi `project/src/docs/experiments/HUMAN_SIGNALS.md` icine yaz; ozellikle Run #97 sonrasi waiting veya game-over ekranindan tek `tap/click` ile baslayan run'in artik ayni basisin oldugu noktaya istemsiz steering vermedigini, pointer release sonrasi steering'in geri geldigini, intentional held-pointer start/retry'nin `180ms` sonra akici sekilde hareketi devralabildigini, blur-sonrasi fresh movement resume'in gereksiz `180ms` held-delay'e dusmedigini, compact waiting/game-over HUD'un validation affordance'larini gizleyip gizlemedigini, Run #99 sonrasi progress satirlarinin tamamlanmis sample'i durust yansitip yansitmadigini, `10-11s` collision-grace fade'in cheap hit'leri yumusatip yumusatmadigini ve center-overlap death guidance'in sahte lane uretip uretmedigini kontrol et.

Run mode: `stabilization`

## Why This Is Next

Haftalik strateji hala `Human-Proven Survival Core`. Run #79-99 hattinda input/pause, freeze semantigi, visible-arena fairness, `20s+` chase, death guidance/readability, `10-11s` collision-grace fade, waiting/game-over HUD, blur-sonrasi fresh movement resume, pointer start/retry steering ve validation sample semantigi source'ta iyilesti; fakat `AUDIT.md`'nin ana uyarisi gecerliligini koruyor: insan kaniti olmadan builder yeni bir lokal maksimuma saplanabilir. Bu tur Run #99 in-game progress satirlarini da tamamlanmis run sayisina hizaladi; simdi bu ve onceki gameplay/UX fix'lerinin insan hissiyle test edilmesi gerekiyor.
...
- interactive runtime yine yoksa pause/resume/held-input mikro-fix zincirine, Run #87 `20s+` chase tuning'ine, Run #88-99 freeze/visible-arena fairness/death-callout/grace-cutoff/center-guidance/HUD/fresh-resume/pointer-start-steering/validation-export-count/completed-run-HUD yuzeylerine bir tur daha donme; yeni ve dar bir gameplay problemi sec
...
- validation export veya completed-run HUD semantigi kapanmisken ayni yuzeye yeni churn uretme
