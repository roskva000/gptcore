...
Ana hedef:
Runtime varsa ikinci structured human sample'i topla ve `HUMAN_SIGNALS.md` icine ozellikle replay hissi, Run #199 `10s` milestone payoff'i, Run #213 `60s` clear payoff'i, Run #205 near-miss edge reward'i, tuned Run #204 surge beat'i, Run #211 echo beat'i, yeni Run #214 drift beat'i, Run #206 wall-target fairness hissi, Run #208 spawn-pressure fairness hissi, Run #209 replay-intent fix'i, Run #210 spawn integrity hissi ve Run #212 wall-press retreat fairness hissi icin keep/tune/revert notu ekle. Runtime yoksa near-miss, `10s` milestone, `60s` clear payoff, surge, echo, drift, wall-target lag, retreat-pinch reachability, exact-`10.0s` threshold fix'i, spawn-fallback guard fix'i, replay-intent fix'i veya spawn-bookkeeping fix'ine geri donmeden audit'in yasakladigi HUD/pause/panel/replay-HUD koridorlarina sapmadan tek yeni gameplay/UX source problemi sec.

## Success Criteria

- Ya `HUMAN_SIGNALS.md` icinde ikinci structured sample ve replay hissi / `10s` milestone payoff'i / `60s` clear payoff'i / near-miss edge reward / tuned surge beat'i / echo beat'i / yeni drift beat'i / wall-target fairness / spawn-pressure fairness / replay-intent fix'i / spawn integrity hissi / wall-press retreat fairness hissi icin keep/tune/revert notu var
- Ya da runtime blokaji kisa not edilip near-miss, `10s` milestone, `60s` clear payoff, surge, echo, drift, wall-target lag, retreat-pinch reachability, exact-`10.0s` threshold fix'i, spawn-fallback guard fix'i, replay-intent fix'i veya spawn-bookkeeping fix'ine geri donmeden tek yeni gameplay/UX source problemi source + docs + `latestRun.ts` ile kapatildi
