...
Ana hedef:
Run #125-#126 sonrasi death ve pause overlay'lerinin artik daha sakin hissedilip hissedilmedigini hedefli bir manuel sample ile dogrula.
...
- Olası aday: game-over sirasinda sag ust `Session snapshot` yogunlugu veya `60s clear.` satirinin gorsel agirligi; fakat tek bir problem sec ve fairness zincirine kayma.
- `npm run telemetry:check` ve `npm run build` ile dogrula.

## Success Criteria

- `HUMAN_SIGNALS.md` icinde Run #125-#126 sonrasi death/pause readability odakli ikinci sample var
