...
Son run sonucu: `integration` modunda run signature family opening boyunca yasayan bir `RUN FEEL` HUD paneli kazandi; intro + opening cue + spawn-profili bias + reminder zinciri artik tek bakista daha toplu gorunuyor.

Haftalik karar:
- run signature family artik intro, opening cue, `RUN FEEL` paneli, spawn-profili opening runtime, reminder ve death/rematch zincirinde daha bagli ama henuz gercek oyuncu hissi kanitlanmadi
...
Ana tema:
**Run signature family'nin intro + opening cue + `RUN FEEL` paneli + spawn-profili opening runtime + mid-run reminder + retry zincirinin gercekten hissedilip hissedilmedigini kanitla; gerekiyorsa yalniz tek bir signature surface'ini daha derinlestir.**

Hedef:
`PINPOINT / WEAVE / RUSH` rotasyonu oyuncu tarafinda sadece label degil, okunur bir run kimligi gibi hissettirsin. Bu tur opening runtime ilk `2-3` spawn icinde daha karakterli hale geldi ve yeni `RUN FEEL` paneli bu pencereyi HUD'a tasidi; siradaki run intro, opening cue, panel, spawn-profili opening bias, reminder, obstacle/HUD renk dili ve death/rematch hook'un browser veya net manuel gozlemde gercekten fark uretip uretmedigini test etmeli. Fark hala yumusaksa yalniz tek bir surface'i derinlestir: ornegin `RUN FEEL` panelinin siddeti/yerlesimi, opening cue suresi veya reminder siddeti.
...
2. intro, ilk collision-ready opening cue, `RUN FEEL` paneli, ilk `0-8.8s` spawn-profili opening bias, `6.2-8.8s` reminder, obstacle/HUD renk dili ve death/rematch hook'tan hangisinin gercekten hissedildigini veya bosta kaldigini tek tek not et
...
- intro/opening-cue/`RUN FEEL` paneli/spawn-profili opening-bias/reminder/readability/retry zincirinden hangisinin gercekten ise yaradigi veya bosta kaldigi net tespit edilir
