...
- Run #178 validation-export status drift'ini kapatti; bunu yeni validation/readiness framework'u veya copy churn'u acmak icin bahane yapma.
...
- Run #178 validation export status line'larinin current vs stale farki

Beklenen cikti:
- `project/src/docs/experiments/HUMAN_SIGNALS.md` icinde tarihli ikinci sample
- opener fairness, near-miss hissi, retry/start kontrolu, game-over okunurlugu ve validation-status anlasilirligi icin kisa karar notlari
...
Ana hedef:
Ayni overlay/mobile/near-miss/validation ve same-edge fairness koridorlarina donmeden tek yeni gameplay veya UX source bug'i sec ve kapat. Seed `#3` ve seed `#7` kapali; fallback adayi yeni bir spawn-pressure / obstacle readability / launch-retry truth kusuru veya baska tekil gameplay baskisi olmali. Ancak once trace cikar; global cutoff / framework denemesiyle baslama.
...
- Run #178'i yeni validation lifecycle/readiness paketi icin bahane yapma; freshness truth kapandi, yeni sistem acma.
