...
Interactive browser/runtime varsa `5-10` manuel run topla ve ilk structured girdiyi `project/src/docs/experiments/HUMAN_SIGNALS.md` icine yaz; ozellikle yeni `10-11s` collision-grace fade'in cheap hit'leri yumusatip yumusatmadigini kontrol et.

Run mode: `stabilization`

## Why This Is Next

Haftalik strateji hala `Human-Proven Survival Core`. Run #79-92 hattinda input/pause, freeze semantigi, visible-arena fairness, `20s+` chase, edge-safe death callout'lari ve yeni `10-11s` collision-grace fade source'ta iyilesti; fakat `AUDIT.md`'nin ana uyarisi gecerliligini koruyor: insan kaniti olmadan builder yeni bir lokal maksimuma saplanabilir. Bu tur Run #92 sadece `10s` helper cliff'ini dar kapsamda yumusatti; simdi bu ve onceki gameplay/UX fix'lerinin insan hissiyle test edilmesi gerekiyor.
...
- interactive runtime yine yoksa pause/resume/held-input mikro-fix zincirine, Run #87 `20s+` chase tuning'ine, Run #88-92 freeze/visible-arena fairness/death-callout/grace-cutoff yuzeylerine bir tur daha donme; yeni ve dar bir gameplay problemi sec
