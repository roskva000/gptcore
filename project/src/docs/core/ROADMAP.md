...
- headed runtime yoksa telemetry/copy veya ayni opening-fairness paketini buyutmeden tek bir dar gameplay/UX bug'ini kapat; Run #95 focus-loss movement resume, Run #97 pointer start/retry steering kilidi ve Run #98-100 validation/export/HUD/log counting semantigi kapandi, bu yuzeylere hemen geri donmeden yeni fallback adayi secilmelidir
...
- manual sample gelmezse validation/export/HUD/log counting yuzeyine de geri donme; `20s+` chase'i tekrar acmadan freeze/cull, obstacle reuse, compact telemetry/support-strip, focus-loss/input, pointer start/retry steering, center-overlap death guidance ve visible-arena fairness disinda tek bir dar source bug'i sec
