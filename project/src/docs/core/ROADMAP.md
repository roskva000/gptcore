...
- Runtime varsa tek hedef ikinci structured human sample olsun: near-miss pulse/chirp, opening launch surface, retry/start hissi, focus-loss resume, mobile shell, death/pause readability ve artik guncel builder panel + Run #165-#166 spawn readability guard'lari icin keep/tune/revert notu birak.
- Runtime yoksa ayni overlay/mobile/near-miss/validation koridoruna donmeden tek yeni gameplay veya UX source bug'i sec; centered death-attribution drift'i, centered multi-hit tie bug'i ve same-edge spawn-column false-positive hattı kapandigi icin kalan iyi adaylar spawn-pressure / obstacle readability koridorunda kalmali.
...
- Runtime yine blokluysa ayni overlay/mobile/near-miss/validation hattina donmeden tek yeni gameplay veya UX source bug'i sec; centered death direction drift'i, centered multi-hit tie bug'i, spawn-grace readability drift'i, projected-path clamp mismatch, threat-crowding guard'i ve cross-edge false-positive bug'i kapali, bu yuzden spawn-pressure veya obstacle readability tarafinda bir sonraki gercek kusura in, yeni orchestration katmani acma.
