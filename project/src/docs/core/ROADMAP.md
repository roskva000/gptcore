...
- headed runtime yoksa telemetry/copy veya ayni opening-fairness paketini buyutmeden tek bir dar gameplay/UX bug'ini kapat; Run #92 `10-11s` spawn collision-grace fade'i ekledi, bu yuzey tekrar buyutulmadan yeni fallback adayi secilmelidir
...
- manual sample notlarina gore fairness, replay, held resume guard'lari, projected-path wall-edge davranisi, obstacle reuse, `10-11s` grace fade, death readability veya control tarafinda en yuksek etkili dar bug fix'i sec
- manual sample gelmezse `20s+` chase'i tekrar acmadan freeze/cull, obstacle reuse ve visible-arena fairness disinda tek bir dar source bug'i sec
