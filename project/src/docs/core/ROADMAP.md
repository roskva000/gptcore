...
- `Human-Proven Survival Core` icin ilk manuel sample'i topla; held start/retry/resume, focus-loss sonrasi pointer refocus-resume guard'i, yeni held movement release guard'i, pointer steering, offscreen hit guard, `11px` collider, pooled obstacle reuse/cull temizligi, `20s+` chase ve audio feedback parity notlarini kaydet
...
- manual sample notlarina gore fairness, replay, held resume guard'lari, obstacle reuse veya control tarafinda en yuksek etkili dar bug fix'i sec
