...
- `Human-Proven Survival Core` icin ilk manuel sample'i topla; held start/retry/resume, focus-loss sonrasi pointer refocus-resume guard'i, blur-sonrasi fresh movement resume davranisi, pause sirasinda frozen kalan spawn-grace readability, yeni `10-11s` collision-grace fade, projected-path forward-pressure secimi, pointer steering, `11px visible-arena hit margin`, `Enter` primary action butunlugu, pooled obstacle reuse/cull temizligi, compact waiting/game-over telemetry, support-strip hiyerarsisi, Run #87 sonrasi `20s+` chase, death lane/readability ve audio feedback parity notlarini kaydet
- Run #117 ile held `Space` / `Enter` auto-repeat artik waiting, paused ve game-over fazlarinda ikinci bir primary action tetiklemiyor; ilk manuel sample'da death tableau okunurken veya pause overlay'i acikken tus basili kalsa bile otomatik retry/resume olmadigini not et
- Run #116 ile `Enter` da gameplay capture altinda; ilk manuel sample'da side panel veya shell focus'u kaymis olsa bile `Enter` start/retry/resume'un panel toggle yerine oyunu tetikledigini not et
- Run #115 ile validation export sadece stable fazlarda ve completed run sample'i varken aciliyor; ilk manuel sample'da `V` hotkey'inin playing/paused sirasinda guard mesaji verdigini, waiting/game-over ekraninda ise export'un normal calistigini not et
- Run #114 ile pointer steering hedefi oyuncunun erisebildigi arena icine clamp'lendi; ilk manuel sample'da pointer/touch drag arena disina kaysa bile wall-edge steering'in artik duvara bosuna itilmeden erisilebilir kacis eksenine aktigini not et
- Run #113 ile center-overlap death'lerde `RESET CENTER` guidance artik gorsel olarak da notr; ilk manuel sample'da merkez overlap olumlerinde escape guide'in sahte yukari/sag/sol lane uretmeden oyuncu merkezinde kaldigini not et
...
- validation export artik playing/paused fazinda veya sifir completed run ile kaydedilmiyor; export yalnizca stable sample durumunu temsil ediyor
- `Enter` artik Space ile ayni primary-action capture hattinda; shell focus kaymasi start/retry/resume akisini yutmamali
- held `Space` / `Enter` auto-repeat artik waiting, paused ve game-over overlay'lerinde ikinci bir primary action uretemiyor; taze key press davranisi korunuyor
...
- pointer/touch hedefi arena disina kaysa bile steering artik oyuncunun erisebildigi arena icindeki clamp'li hedefe aktigi icin wall-edge drag kontrolu imkansiz dis lane'e hiz kaybetmiyor
...
- centered overlap death'lerde escape guide artik sahte yukari kacis oku cizmiyor; marker/label notr merkez fallback'inde kaliyor
...
- manual sample notlarina gore fairness, replay, held resume guard'lari, held primary-key repeat davranisi, `Enter` start/retry/resume butunlugu, player-reachable edge target clamp, wall-pinned corner spawn secimi, projected-path wall-edge davranisi, obstacle reuse, `10-11s` grace fade, edge death readability veya control tarafinda en yuksek etkili dar bug fix'i sec
- manual sample gelmezse validation/export/HUD/log counting, Run #115 validation export guard'i, Run #116 `Enter` capture fix'i, Run #117 primary-key repeat guard'i, opening fairness, Run #101 edge-target clamp, Run #102 wall-pinned velocity clamp, Run #103 non-pointer start/resume steering guard'i, Run #104 canli run-time timing, Run #105 game-over freeze, Run #106 pause-clock freeze, Run #107 pre-spawn cull cleanup, Run #108 browser control guard'i, Run #109 edge-callout horizontal clamp, Run #110 fatal threat attribution, Run #111 death tableau visual priority, Run #112 game-over held-input retry release guard, Run #113 center-overlap neutral escape guide veya Run #114 pointer steering reachable clamp yuzeyine geri donme; `20s+` chase'i tekrar acmadan baska tek bir dar source bug'i sec
...
- validation export phase/readiness guard yuzeyine hemen geri donme
...
- Run #113 center-overlap neutral escape guide yuzeyine sample olmadan tekrar donme
- Run #114 pointer steering reachable clamp yuzeyine sample olmadan tekrar donme
