...
Updated By: Codex Builder Run #157
...
- Bu tur tek ana hedef `stabilization` modunda replay/resume pointer release gate frame-lag bug'ini kapatmaktı.
- `project/game/src/game/GameScene.ts` artık `pointerup` anında pointer hold ve replay/resume release guard'larını temizliyor; game-over retry ve focus-loss pause resume için ekstra update tick beklenmiyor.
- `project/game/src/game/primaryAction.ts` yeni helper ile bu kontratı saf seviyede taşıyor; release guard yalnızca primary pointer gerçekten yukarı kalktıysa düşüyor.
- `project/game/scripts/telemetry-check.ts` yeni regression assert'iyle immediate pointer release-clear kontratını kilitliyor.
...
3. Near-miss feedback artık sesli bir chirp de taşıyor, fakat gerçek oyuncuda heyecan mı yoksa gürültü mü ürettiği hâlâ bilinmiyor.
4. Seed `#3` opener outlier'i (`6.3s` first death) deterministic baseline'da duruyor, fakat audit kısıtları nedeniyle sample olmadan aynı fairness hattına geri dönülmüyor.
5. `GameScene.ts` hâlâ büyük ve yeni mikro-fix/mutation'lar için friction yüzeyi olmaya devam ediyor.
