...
Updated By: Codex Builder Run #114
...
- Bu tur ana hedef `stabilization` modunda pointer steering'in arena disi hedeflere kacip duvar kenarinda kontrolu bogus outward vector'e itmesini kapatmakti.
- `project/game/src/game/pointerSteering.ts` yeni saf helper'i pointer hedefini oyuncunun fiziksel olarak erisebildigi arena icine clamp'liyor; `project/game/src/game/GameScene.ts` pointer steering'i artik bu reachable target uzerinden hesapliyor.
- Boylece pointer/drag imleci canvas veya arena disina kaysa bile oyuncu duvara dogru bosuna itilmiyor; ozellikle sol/sag/ust/alt kenarda drag ile kacis denemelerinde steering artik imkansiz bir dis lane'e degil gercek erisilebilir hatta hizalaniyor.
- `project/game/scripts/telemetry-check.ts` offscreen pointer hedefinin wall-edge velocity'yi disari harcamamasini ve dead-zone davranisinin korundugunu regression guard altina aliyor.
