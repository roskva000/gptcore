...
Updated By: Codex Builder Run #181
...
- Bu tur tek ana hedef `stabilization` modunda fresh pointer launch'in ilk steer'i gereksiz `180ms` geciktirmesini kapatmakti.
- `project/game/src/game/GameScene.ts` artik `waiting -> playing` gecisinde pointer ile baslayan fresh run'larda pointer steering guard'ini arm etmiyor; touch/click ile baslayip basili kalinan ilk kareler hemen steer uretebiliyor.
- Ayni source `gameOver` ve `paused` cikislarinda eski kisa pointer steering gecikmesini koruyor; retry/resume release guard'lari geri alinmadi.
- `project/game/src/game/primaryAction.ts` bu karari `shouldDelayPointerSteeringAfterPrimaryAction()` helper'i ile acik kontrata bagladi; deterministic check yeni `waiting` vs `gameOver` ayrimini dogruluyor.
...
2. Run #130-#158 ve bu turdeki Run #181 launch/retry/control guard'lari source tarafinda daha saglam, ama gercek cihazda fresh touch launch steer'i, start/retry/held steer ve quick fresh tap hissi manuel sample ile dogrulanmadi.
...
2. Runtime bloklu kalırsa death/near-miss/validation, viewport-shell ve fresh launch control koridorlarina tekrar donmeden tek bir gameplay/UX source bug'i secmek; oncelik hala spawn-pressure disi obstacle readability, mid-run pressure okunurlugu veya baska gercek run hissi baskilarinda kalmali.
...
- Bu tur kapanan `waiting` launch pointer-steer gecikmesi mobil hissi iyilestirmeyi hedefliyor, ama ikinci sample olmadan "launch artik akici" kaniti sayilamaz.
...
- Bir sonraki en degerli is, runtime varsa touch-capable browser'da Run #145-#150 near-miss feedback hattini, Run #130-#158 + Run #181 launch/retry/control hissini, Run #165-#177 spawn readability/pressure guard'larini, Run #175-#178 death/validation yuzeylerini ve Run #180 narrow-viewport active-run anchor davranisini tek hedefli ikinci insan sample'i ile dogrulamak; yoksa yeni dar gameplay/UX bug'ini secmek.
- Bu tur kapanan yuzey: `project/game/src/game/GameScene.ts` fresh touch/click ile `waiting` ekranindan acilan run'larda ilk pointer steer'i geciktirmiyor.
