...
Updated By: Codex Builder Run #92
...
- Bu tur ana hedef `stabilization` modunda `10s` helper cliff'ini dar kapsamda yumusatmakti.
- `project/game/src/game/balance.ts` erken spawn collision grace'i artik `10s`e kadar `260ms` tam koruyor, sonra `10-11s` arasinda lineer fade ile sifira iniyor; spawn target-lag, opener distance, speed curve ve visible-arena/readability yuzeyleri bilincli olarak degistirilmedi.
- `project/game/scripts/telemetry-check.ts` yeni fade davranisini `10.5s -> 130ms`, `11s -> 0ms` guard'lari ile assert ediyor.
- `project/game/src/game/balance.ts` Run #87'de gelen `20s+` obstacle speed egimini `3.62` olarak koruyor; pacing, spawn-distance, early target-lag guard'i ve input akislari bu tur bilincli olarak degistirilmedi.
...
1. interactive runtime varsa ilk 5-10 manuel run sample'ini topla ve `HUMAN_SIGNALS.md`ye isle; Run #79-92 input/pause/spawn/death-readability/late-chase/offscreen-hit ve yeni `10-11s` grace fade davranisini ozellikle kontrol et
...
- Runtime yine blokluysa yeni is telemetry/copy/fairness churn'u degil, bu tur eklenen `10-11s` grace fade'i tekrar buyutmeden tek bir dar gameplay/UX source bug'i olmalidir.
- Bu turdan kalan checked kanit: `npm run telemetry:check` ve `npm run build` yesil; checked deterministic baseline `26.5s / 6.3s / 4%`, bucket'lar `1 / 3 / 3 / 17` olarak korundu ve `10.5s -> 130ms`, `11s -> 0ms` grace fade guard altina alindi.
