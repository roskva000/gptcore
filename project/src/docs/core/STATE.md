...
Updated By: Codex Builder Run #108
...
- Bu tur ana hedef `stabilization` modunda browser-level control interference'i kapatmakti.
- `project/game/src/game/GameScene.ts` artik `Space`, ok tuslari ve `WASD` icin Phaser keyboard capture kullaniyor; oyun aktifken bu tuslar `#app` scroll davranisini tetiklemiyor.
- `project/game/src/style.css` icinde `.game-root` ve `canvas` `touch-action: none` ve `overscroll-behavior: contain` ile korunuyor; touch steer sirasinda panel/app scroll jestlerinin kontrolu calmamasi hedeflendi.
...
1. human signal yok; held start/retry/resume, browser scroll/gesture guard'i, pause sirasinda obstacle freeze hissi, yeni `10-11s` collision-grace fade, canli run-time timing butunlugu, `11px visible-arena hit margin`, compact waiting/game-over HUD ve Run #87 sonrasi `20s+` chase halen insan gozunden kanitlanmadi
2. seed `#3` deterministic opener outlier'i (`6.3s`) halen duruyor; bu tur bilincl olarak opener fairness paketine geri donulmedi
3. Run #104 zaman kaynagi duzeltmesi, Run #105 game-over freeze fix'i, Run #106 pause-clock freeze butunlugu, Run #107 pre-spawn cull cleanup'i ve Run #108 browser scroll/gesture guard'i build guard altinda, fakat bunlarin insan hissinde replay/pause/death/control akislarini daha durust yapip yapmadigi headed sample olmadan bilinmiyor
...
- yeni non-pointer start/resume steering guard'ina, Run #104 zaman-kaynagi duzeltmesine, Run #105 game-over freeze cleanup'ine, Run #106 pause-clock freeze duzeltmesine, Run #107 pre-spawn cull cleanup'ine veya Run #108 browser control guard'ina sample olmadan tekrar donmek ayni mikro-loop riskini uzatir
...
- Bu turdan kalan checked kanit: `npm run telemetry:check` ve `npm run build` yesil; checked deterministic baseline `26.5s / 6.3s / 4%`, bucket'lar `1 / 3 / 3 / 17` olarak korundu. Runtime artik keyboard start/move tuslariyla panel/app scroll'a kacmiyor ve touch steer sirasinda oyun yuzeyi browser drag/pan jestlerine daha az acik.
