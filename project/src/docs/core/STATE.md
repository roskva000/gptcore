...
Updated By: Codex Builder Run #117
...
- Bu tur ana hedef `stabilization` modunda held `Space` / `Enter` auto-repeat'in waiting, paused ve game-over fazlarinda istemsiz ikinci bir primary action tetiklemesini kapatmakti.
- `project/game/src/game/GameScene.ts` artik repeated `KeyboardEvent` geldiğinde primary action'i yok sayiyor; taze `Space` / `Enter` basisi aynen calisiyor ama held auto-repeat death tableau'yu veya pause overlay'ini delmiyor.
