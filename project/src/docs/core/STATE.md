...
Updated By: Codex Builder Run #101
...
- Bu tur ana hedef `stabilization` modunda obstacle hedefleme noktasini oyuncunun gercekte ulasabilecegi arena sinirlariyla hizalamakti.
- `project/game/src/game/GameScene.ts` spawn target lag hesabini artik `PLAYER_COLLISION_RADIUS` marjli arena icine clamp'liyor; obstacle'lar duvar kenarinda oyuncunun merkezin hic gidemeyecegi `x=0/800`, `y=0/600` noktalara nisan almiyor.
- `project/game/scripts/telemetry-reports.ts` ayni clamp'i deterministic proxy'ye de tasidi; runtime ve snapshot pathing hizasi korunurken edge-path hedeflemesi tekrar ayni fiziksel alana baglandi.
- `project/game/src/game/spawn.ts` icindeki yeni `clampPointToArena()` helper'i bu davranisi tek yerde tutuyor ve `project/game/scripts/telemetry-check.ts` oyuncu-erisilebilir arena clamp'i icin regression guard ekliyor.
...
3. duvar-kenari obstacle pathing artik oyuncu-erisilebilir hedefe clamp'lense de bunun insan hissinde chase'i daha okunur mu yoksa fazla bagislayici mi yaptigi headed sample olmadan bilinmiyor
...
- Runtime yine blokluysa yeni is telemetry/copy/fairness churn'u degil, bu tur kapanan validation export/HUD/log sample semantigi, onceki pointer/focus-loss yuzeyleri veya yeni edge-target clamp'e geri donmeden tek bir dar gameplay/UX source bug'i secmek olmalidir.
- Bu turdan kalan checked kanit: `npm run telemetry:check` ve `npm run build` yesil; checked deterministic baseline `26.5s / 6.3s / 4%`, bucket'lar `1 / 3 / 3 / 17` olarak korundu ve obstacle target path'i oyuncu-erisilebilir arena icine hizalandi.
