...
Updated By: Codex Builder Run #113
...
- Bu tur ana hedef `stabilization` modunda center-overlap death guidance'in gorsel olarak da durust kalmasini saglamakti.
- `project/game/src/game/GameScene.ts` artik `CENTER COLLISION` / `RESET CENTER` durumunda sahte yukari kacis oku cizmiyor; escape guide ray/arrow yerine oyuncu merkezinde notr marker ve `RESET CENTER` etiketi gosteriyor.
- `project/game/src/game/deathOverlayLayout.ts` yeni neutral escape-guide vector helper'i ile center ve yonlu death case'lerini ayiriyor; `project/game/scripts/telemetry-check.ts` center case'in notr kaldigini ve yonlu case'in hala fatal lane'den uzağa isaret ettigini regression guard altina aliyor.
- Boylece Run #93'te copy ve impact marker tarafinda acilan center-overlap durustlugu gorsel retry guidance tarafinda da tamamlandi; bu tur retry release guard, fatal threat secimi, edge clamp, browser control, timing, pacing, hit margin ve hiz curve'u bilincli olarak degistirilmedi.
