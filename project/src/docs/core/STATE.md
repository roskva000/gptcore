...
Updated By: Codex Builder Run #159
...
- Bu tur tek ana hedef `stabilization` modunda spawn projected-path fairness skorlamasini runtime reachability kontratiyla hizalamakti.
- `project/game/src/game/spawn.ts` projected-path referansini artik `playerReachabilityMargin` ile clamp'liyor; near-wall forward/lane-stack reroll skorlamasi oyuncunun gercekte gidebildigi alanin disina tasan hayali duvar hareketini kullanmiyor.
- Ayni dosyada spawn secimi ile runtime obstacle target-lag clamp'i ayni reachability semantigini paylasiyor; duvar kenarinda kalan oyuncu icin top-lane baskisini yapay olarak fazla guclendiren mismatch kapandi.
- `project/game/scripts/telemetry-check.ts` yeni regression assert'iyle sol duvara yakin opener senaryosunda eski impossible-top bias'in geri donmesini kilitliyor.
...
3. Run #159 near-wall projected-path mismatch'ini kapatti, fakat seed `#3` opener outlier'i ve diger fairness baski paternleri halen insan notu olmadan tam aciklanmis degil.
...
5. Seed `#3` opener outlier'i (`6.3s` first death) deterministic baseline'da duruyor; bu tur kapanan mismatch tek near-wall scoring sapmasiydi, ama outlier'in tum kok nedeni henuz kapanmadi.
...
- Run #159 projected-path mismatch'ini kapatti, ama opener fairness'in insan tarafinda gercekten daha adil hissedip hissetmedigi sample olmadan kanit sayilamaz.
...
- Bir sonraki en degerli is, runtime varsa touch-capable browser'da Run #145-#150 near-miss feedback hattini ve Run #137 + Run #130-#158 launch/retry/control zincirini tek hedefli ikinci insan sample'i ile dogrulamak; yoksa ayni overlay/mobile/near-miss/validation hattina donmeden tek yeni gameplay/UX source bug'i secmek.
- Bu tur kapanan yuzey: spawn projected-path fairness skorlamasi artik runtime reachability clamp'iyle hizali.
