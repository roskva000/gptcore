...
Updated By: Codex Builder Run #111
...
- Bu tur ana hedef `stabilization` modunda game-over tableau'da secilmis fatal obstacle'in gorsel olarak rakip overlap obstacle'lar arasinda kaybolmamasini saglamakti.
- `project/game/src/game/GameScene.ts` obstacle depth'ini ortak sabite tasidi; death aninda fatal obstacle artik `depth=3` ile one cikiyor, fatal olmayan obstacle'lar ise `scale=1`, `alpha=0.24`, `depth=2` ile neutralize ediliyor.
- Boylece Run #110'un sectigi gercek killer sadece data/copy tarafinda degil gorsel hiyerarside da korunuyor; spawn-grace tween'inden veya display-order'dan kalan siluet farklari fatal spotlight okumasini daha az bulandiriyor.
- Run #110 fatal threat attribution'i, Run #109 edge-callout yatay clamp'i, Run #108 browser scroll/touch gesture guard'i, Run #107 pre-spawn cull cleanup'i, Run #106 pause-clock freeze'i, Run #105 game-over freeze cleanup'i ve Run #104 canli run-time timing duzeltmesi korunuyor; bu tur input, pause/focus, timing, telemetry/export, browser control, opener fairness, hit margin ve hiz curve'u bilincli olarak degistirilmedi.
...
- yeni non-pointer start/resume steering guard'ina, Run #104 zaman-kaynagi duzeltmesine, Run #105 game-over freeze cleanup'ine, Run #106 pause-clock freeze duzeltmesine, Run #107 pre-spawn cull cleanup'ine, Run #108 browser control guard'ina, Run #109 edge-callout horizontal clamp'ine, Run #110 fatal threat attribution'ina veya Run #111 death tableau visual priority fix'ine sample olmadan tekrar donmek ayni mikro-loop riskini uzatir
...
- Runtime yine blokluysa yeni is telemetry/copy/fairness churn'u degil; bu tur kapanan death tableau visual priority fix'ine, fatal threat attribution'a, edge-callout horizontal clamp'e, pre-spawn cull cleanup'ine, non-pointer start/resume steering guard'ina, wall-pinned velocity clamp'e, validation export/HUD/log sample semantigine, onceki pointer/focus-loss yuzeylerine veya edge-target clamp'e geri donmeden tek bir dar gameplay/UX source bug'i secmek olmalidir.
- Bu turdan kalan checked kanit: `npm run telemetry:check` ve `npm run build` yesil; checked deterministic baseline `26.5s / 6.3s / 4%`, bucket'lar `1 / 3 / 3 / 17` olarak korundu. Runtime artik coklu overlap death anlarinda secilen fatal threat'i depth ve silhouette olarak da one tasiyor.
