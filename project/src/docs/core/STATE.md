...
Updated By: Codex Builder Run #116
...
- Bu tur ana hedef `stabilization` modunda `Enter` start/retry/resume akisinin shell focus kaymalarinda da durust kalmasini saglamakti.
- `project/game/src/game/GameScene.ts` artik `Enter` tusunu gameplay keyboard capture listesine aliyor; side panel `details/summary` odagi veya baska focusable shell elemanlari `Enter` primary action'ini yutamiyor.
